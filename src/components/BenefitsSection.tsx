...
const BenefitsSection = () => {
  const benefits = [{
    ...
    details: ["Automated request handling", "Self-service ordering", "Reduced labor expenses", "Better staff efficiency"]
  }, {
    icon: "💬",
    title: "Requests & Ordering in One Tap",
    description: "Guests can ask for the bill, change music, or order — stress-free",
    details: ["QR code simplicity", "Instant communication", "No more hand waving", "Complete guest control"]
  }, {
    icon: "📊",
    title: "Real-Time Analytics",
    description: "Track orders, preferences, and optimize your operations",
    details: ["Order tracking insights", "Guest behavior patterns", "Peak hour analysis", "Revenue optimization"]
  }];
  return <section className="py-20 bg-gradient-warm">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {benefits.map((benefit, index) => <Card key={index} className="group p-8 text-center hover:shadow-elegant transition-all duration-300 hover:scale-105 bg-card/80 backdrop-blur-sm">
              <div className="mb-6">
              ...
              <div className="space-y-3">
                {benefit.details.map((detail, detailIndex) => <div key={detailIndex} className="flex items-center gap-3 text-sm p-3 rounded-lg bg-muted/50 group-hover:bg-primary/5 transition-colors">
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          ...
          <div className="space-y-2 mx-[150px]">
            <div className="text-3xl font-bold text-primary">24/7</div>
            <div className="text-sm text-muted-foreground bg-gray-950 mx-0">Always available</div>
          </div>
        </div>
      </div>
    </section>;
